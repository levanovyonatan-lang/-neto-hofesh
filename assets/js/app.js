...
function refreshPWAIconsSilently() {
    try {
        const cb = Date.now();
        fetch('manifest.json?v=258&cb=' + cb, { cache: 'reload' }).catch(() => {});
        fetch('icon-neto-sunglasses.png?v=258&cb=' + cb, { cache: 'reload' }).catch(() => {});
        fetch('icon-neto-sunglasses-white.png?v=258&cb=' + cb, { cache: 'reload' }).catch(() => {});
        fetch('icon-neto-sunglasses-transparent.png?v=258&cb=' + cb, { cache: 'reload' }).catch(() => {});

        document.querySelectorAll('link[rel="apple-touch-icon"], link[rel="icon"]').forEach(link => {
            const baseHref = link.href.split('?')[0];
            link.href = baseHref + '?v=258&cb=' + cb;
        });
        const manifestLink = document.querySelector('link[rel="manifest"]');
        if (manifestLink) {
            manifestLink.href = 'manifest.json?v=258&cb=' + cb;
        }
    } catch (e) { }
}
