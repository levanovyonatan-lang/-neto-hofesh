...
// Holidays data moved to modules/holidays.js
...
function initPWA() {
    if ('serviceWorker' in navigator) {
        ...
        navigator.serviceWorker.register('sw.js?v=10144').then(reg => {
            reg.update();
...
function refreshPWAIconsSilently() {
    try {
        const cb = Date.now();
        fetch('manifest.json?v=10144&cb=' + cb, { cache: 'reload' }).catch(() => { });
        fetch('icon-neto-sunglasses.png?v=10144&cb=' + cb, { cache: 'reload' }).catch(() => { });
        fetch('icon-neto-sunglasses-white.png?v=10144&cb=' + cb, { cache: 'reload' }).catch(() => { });
        fetch('icon-neto-sunglasses-transparent.png?v=10144&cb=' + cb, { cache: 'reload' }).catch(() => { });

        document.querySelectorAll('link[rel="apple-touch-icon"], link[rel="icon"]').forEach(link => {
            const baseHref = link.href.split('?')[0];
            link.href = baseHref + '?v=10144&cb=' + cb;
        });
        const manifestLink = document.querySelector('link[rel="manifest"]');
        if (manifestLink) {
            manifestLink.href = 'manifest.json?v=10144&cb=' + cb;
        }
    } catch (e) { }
}
...
async function loadDailyTipsDatabase() {
    ...
    const fetchUrl = window.location.origin + '/assets/data/daily-tips.json?v=10153';
...
function showMainScreen() {
    ...
    if (!document.getElementById('dino-school-script') && typeof window.startDinoGame !== 'function') {
        ...
        script.src = 'assets/js/games/dino-school.js?v=10144';
...
function selectTarget(id, shouldScroll = true) {
    ...
    if (personalPremium) {
        ...
        if (!personalLabel) {
            ...
            if (absoluteTimer) absoluteTimer.before(personalLabel);
        }
...
function updateDashboard() {
    ...
    if (diff <= 0) {
        ...
        if (isPersonalCountdownPremium(event)) {
        ...
        if (!isAnimatingAbs) {
            ...
            const secsVal = String(Math.floor((diff % 60000) / 1000)).padStart(2, '0');
            const secsEl = document.getElementById('abs-secs');
            if (secsEl && secsEl.textContent !== secsVal) {
                secsEl.textContent = secsVal;
                secsEl.classList.remove('tick-pop');
                void secsEl.offsetWidth; // force reflow
