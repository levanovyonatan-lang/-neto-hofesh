...
function initPWA() {
    if ('serviceWorker' in navigator) {
        ...
        navigator.serviceWorker.register('sw.js?v=10145').then(reg => {
            reg.update();
...
function refreshPWAIconsSilently() {
    try {
        const cb = Date.now();
        fetch('manifest.json?v=10145&cb=' + cb, { cache: 'reload' }).catch(() => { });
        fetch('icon-neto-sunglasses.png?v=10145&cb=' + cb, { cache: 'reload' }).catch(() => { });
        fetch('icon-neto-sunglasses-white.png?v=10145&cb=' + cb, { cache: 'reload' }).catch(() => { });
        fetch('icon-neto-sunglasses-transparent.png?v=10145&cb=' + cb, { cache: 'reload' }).catch(() => { });

        document.querySelectorAll('link[rel="apple-touch-icon"], link[rel="icon"]').forEach(link => {
            const baseHref = link.href.split('?')[0];
            link.href = baseHref + '?v=10145&cb=' + cb;
        });
        const manifestLink = document.querySelector('link[rel="manifest"]');
        if (manifestLink) {
            manifestLink.href = 'manifest.json?v=10145&cb=' + cb;
        }
    } catch (e) { }
}
...
async function loadDailyTipsDatabase() {
    ...
    const fetchUrl = window.location.origin + '/assets/data/daily-tips.json?v=10148';
...
function showMainScreen() {
    ...
    if (!document.getElementById('dino-school-script') && typeof window.startDinoGame !== 'function') {
        ...
        script.src = 'assets/js/games/dino-school.js?v=10145';
...
function saveCustomCountdown() {
    ...
    const emojiStr = emojiInput ? emojiInput.value : 'נ“…';
...
function updateSponsorTexts() {
    ...
    const sponsorOptions = [
        { text: "<b><span style=\"color: #166534;\">׳‘׳•׳ ׳׳₪׳¨׳•׳¥ ׳׳× ׳”׳’׳‘׳•׳׳•׳× ׳©׳׳ ׳¢׳ ׳׳™׳׳•׳ ׳™ ׳›׳•׳— ׳׳˜׳•׳¨׳₪׳™׳.</span></b>", emojis: "נ‹ן¸ג€ג™‚ן¸ נ’×" },
        { text: "<b><span style=\"color: #166534;\">׳‘׳•׳ ׳׳¨׳¡׳§ ׳׳× ׳”׳©׳™׳¢׳׳•׳ ׳¢׳ ׳׳™׳׳•׳ ׳™׳ ׳׳˜׳¨׳™׳₪׳™׳, ׳›׳•׳— ׳•׳—׳‘׳¨׳™׳ ׳—׳“׳©׳™׳</span></b>", emojis: "נ‹ן¸ג€ג™‚ן¸" }
    ];
...
window.renderPrivateTimers = function() {
    ...
    function updateTimers() {
        let html = '<div style="font-size: 16px; font-weight: 800; color: #b45309; text-align: center; width: 100%;">׳”׳¡׳₪׳™׳¨׳•׳× ׳”׳׳™׳©׳™׳•׳× ׳©׳׳ ג±ן¸</div>';
        ...
        window.currentUserProfile.privateTimers.forEach((timer, index) => {
            ...
            html += `
                <div style="background: rgba(255, 255, 255, 0.7); backdrop-filter: blur(5px); border: 1px solid #facc15; padding: 10px; border-radius: 12px; width: 100%; max-width: 320px; box-shadow: 0 4px 10px rgba(0,0,0,0.05); position: relative;">
                    <button onclick="window.deletePrivateTimer(${index})" style="position: absolute; left: 10px; top: 10px; background: none; border: none; color: #ef4444; font-size: 16px; cursor: pointer; padding: 0;">ֳ—</button>
