...
function initPWA() {
    if ('serviceWorker' in navigator) {
        ...
        navigator.serviceWorker.register('sw.js?v=10143').then(reg => {
            reg.update();
...
function refreshPWAIconsSilently() {
    try {
        const cb = Date.now();
        fetch('manifest.json?v=10143&cb=' + cb, { cache: 'reload' }).catch(() => { });
        fetch('icon-neto-sunglasses.png?v=10143&cb=' + cb, { cache: 'reload' }).catch(() => { });
        fetch('icon-neto-sunglasses-white.png?v=10143&cb=' + cb, { cache: 'reload' }).catch(() => { });
        fetch('icon-neto-sunglasses-transparent.png?v=10143&cb=' + cb, { cache: 'reload' }).catch(() => { });

        document.querySelectorAll('link[rel="apple-touch-icon"], link[rel="icon"]').forEach(link => {
            const baseHref = link.href.split('?')[0];
            link.href = baseHref + '?v=10143&cb=' + cb;
        });
        const manifestLink = document.querySelector('link[rel="manifest"]');
        if (manifestLink) {
            manifestLink.href = 'manifest.json?v=10143&cb=' + cb;
        }
    } catch (e) { }
}
...
function handleSponsorClick() {
    ...
    window.open('/fitness.html?v=10143', '_blank');
}
...
async function loadDailyTipsDatabase() {
    ...
    const fetchUrl = window.location.origin + '/assets/data/daily-tips.json?v=10143';
...
function showMainScreen() {
    ...
    if (!document.getElementById('dino-school-script') && typeof window.startDinoGame !== 'function') {
        ...
        script.src = 'assets/js/games/dino-school.js?v=10143';
