...
(function() {
    ...
    const STAGES = [
        /* 0  */ { threshold: 0, title: 'שלב 1: 8:00 בבוקר... למי יש כוח 🥱', bg: 'linear-gradient(to bottom, #bae6fd, #f0f9ff)', gravity: 0.7, jumpForce: -11.5, flyChance: 0, bonusChance: 0, obsSet: ['🚌', '🎒', '⏰'], dinoEmoji: '🦖', dinoFilter: 'none', dinoOpacity: '1' },
        /* 1  */ { threshold: 120, title: 'שלב 2: בוחן פתע ששכחת ממנו! 😱📝', bg: 'linear-gradient(to bottom, #fca5a5, #fee2e2)', gravity: 0.7, jumpForce: -11.5, flyChance: 0.25, bonusChance: 0, obsSet: ['📝', '📋', '📚', '📐'], dinoEmoji: '🦕', dinoFilter: 'none', dinoOpacity: '1' },
        /* 2  */ { threshold: 240, title: 'שלב 3: מלחמת אוכל בקפיטריה! 🍕', bg: 'linear-gradient(to bottom, #fbcfe8, #fdf2f8)', gravity: 0.7, jumpForce: -11.5, flyChance: 0, bonusChance: 0.45, bonusSet: ['🍕', '🥪'], obsSet: ['🗑️', '🪑', '🧹', '📦', '🗄️'], dinoEmoji: '🦖', dinoFilter: 'drop-shadow(0 2px 5px rgba(251,191,36,0.6))', dinoOpacity: '1', objective: 'תאספו אוכל! 🍕🥪' },
        /* 3  */ { threshold: 380, title: 'שלב 4: מישהו פוצץ את הברזייה! 🌊', bg: 'linear-gradient(to bottom, #60a5fa, #1d4ed8)', gravity: 0.45, jumpForce: -9.5, flyChance: 0.3, flySet: ['🐟', '🌊', '💧'], bonusChance: 0, obsSet: ['🚰', '🪣', '🧹', '🚧', '⚠️'], dinoEmoji: '🐊', dinoFilter: 'none', dinoOpacity: '1' },
        /* 4  */ { threshold: 520, title: 'שלב 5: שיעור ספורט ממוצע! 🏀', bg: 'linear-gradient(to bottom, #86efac, #dcfce7)', gravity: 0.6, jumpForce: -10.5, flyChance: 0.35, flySet: ['🏸', '🥏', '🏐', '🎾'], bonusChance: 0, obsSet: ['⚽', '🏀', '🎳', '🏋️‍♂️', '🚧', '👟'], dinoEmoji: '🦖', dinoFilter: 'none', dinoOpacity: '1' },
        /* 5  */ { threshold: 680, title: 'שלב 6: הטיול השנתי המקולל 🌵', bg: 'linear-gradient(to bottom, #fde047, #ea580c)', gravity: 0.7, jumpForce: -11.5, flyChance: 0.2, flySet: ['🦅', '🦇', '🐝', '🦟', '🦅'], bonusChance: 0, obsSet: ['🐍', '🦂', '🌵', '🥾'], dinoEmoji: '🦕', dinoFilter: 'none', dinoOpacity: '1' },
        /* 6  */ { threshold: 950, title: 'שלב 7: ניסוי מדעים יצא משליטה 💥', bg: 'linear-gradient(to bottom, #a78bfa, #c026d3)', gravity: 0.7, jumpForce: -11.5, flyChance: 0.3, flySet: ['🧪', '🦠', '🛸', '⚡', '☄️'], bonusChance: 0, obsSet: ['🧪', '🔬', '💥', '🦠'], dinoEmoji: '🐉', dinoFilter: 'hue-rotate(280deg) saturate(2)', dinoOpacity: '1' },
        /* 7  */ { threshold: 1250, title: 'שלב 8: שיעור חלון! התגנבות יחידים... 🎧', bg: 'linear-gradient(to bottom, #fbcfe8, #f43f5e)', gravity: 0.7, jumpForce: -11.5, flyChance: 0, bonusChance: 0.5, bonusSet: ['🎧', '📱'], obsSet: ['🎒', '🎵', '🎶', '🎮'], dinoEmoji: '🦖', dinoFilter: 'none', dinoOpacity: '1', objective: 'תאספו ציוד! 🎧📱' },
        /* 8  */ { threshold: 1600, title: 'שלב 9: מזגן על 16 - קפוא פה! 🥶', bg: 'linear-gradient(to bottom, #1e293b, #334155)', gravity: 0.75, jumpForce: -12.0, flyChance: 0.25, flySet: ['🌨️', '🌬️', '🧊', '❄️', '🪁'], bonusChance: 0, obsSet: ['☔', '💧', '🌬️', '🌨️', '🌂', '🧊'], dinoEmoji: '🦖', dinoFilter: 'hue-rotate(180deg) brightness(1.3)', dinoOpacity: '1' },
        /* 9  */ { threshold: 2000, title: 'שלב 10: אוי לא, המנהל במסדרון!!! 🚨', bg: 'linear-gradient(to bottom, #ef4444, #7f1d1d)', gravity: 0.7, jumpForce: -11.5, flyChance: 0, bonusChance: 0, obsSet: ['👨‍💼', '👮‍♂️', '🛑', '🚨'], dinoEmoji: '🦖', dinoFilter: 'none', dinoOpacity: '1' },
        /* 10 */ { threshold: 2500, title: 'שלב 11: ננעלת בבית ספר! 🌙👻', bg: 'linear-gradient(to bottom, #020617, #0f172a)', gravity: 0.7, jumpForce: -11.5, flyChance: 0.4, flySet: ['👻', '🦇', '💀', '👽', '🕷️', '🦉'], bonusChance: 0, obsSet: ['🚌', '🎒', '⏰', '📝', '📋', '📚', '📐', '👻', '🔦'], dinoEmoji: '🦖', dinoFilter: 'invert(1) opacity(0.6)', dinoOpacity: '0.6' },
        /* 11 */ { threshold: 3100, title: 'שלב 12: החופש הגדול!!! (המורה עדיין רודפת אחריך 😱)', bg: 'linear-gradient(to bottom, #f97316, #facc15)', gravity: 0.8, jumpForce: -12.5, flyChance: 0.45, flySet: ['✈️', '📓', '📝', '🛸'], bonusChance: 0.3, bonusSet: ['🍉', '🍦', '🍹'], obsSet: ['👩‍🏫', '📝', '⏰', '🏫', '🎈', '✈️'], dinoEmoji: '😎', dinoFilter: 'none', dinoOpacity: '1', objective: 'תאספו פינוקים! 🍉🍦🍹' }
    ];
    ...
    function gameLoop(currentTime) {
        ...
        const STEP_MS = 1000 / 60; // 60Hz physics timestep for buttery smooth 1-step-per-frame parabolic jumps
    ...
    function getGameOverMessage(stageIndex, killerEmoji) {
        const msgMap = {
            '🚌': [
                "האוטובוס נסע בלעדיך, והנהג עוד עשה לך שלום מהחלון. 🚌💨",
                "פספסת את האוטובוס כי עצרת לקשור שרוכים. 🚌",
                "רצת אחרי האוטובוס, אבל הוא עצר רק ברמזור וברח כשקרבת. 🚌💨",
            ...
            '🎒': [
                ...
                "התיק שלך כבד כל כך שפשוט נפלת אחורה בחצר. 🎒",
                "נקרעה לך הכתפייה של התיק מול כל השכבה. פאדיחה. 🎒",
            ...
            '⏰': [
                ...
                "שעון מעורר לא באמת עוזר כשמכבים אותו מתוך שינה. ⏰",
            ...
            '📝': [
                ...
                "הגשת את המבחן בלי לכתוב עליו שם. נפסלת. 📝",
                "המורה הקריאה את התשובה המביכה שלך למבחן מול כולם. 📝",
            ...
            '💧': "עשו עליך פצצת מים בברזייה. קלאסי. 💧",
            '🚰': "המים בברזייה היו בצבע חום... לא רעיון טוב לשתות. 🚰",
            '🪣': "האב בית השאיר פה דלי עם סמרטוט. צללת פנימה. 🪣",
            '🚧': "החלקת על 'זהירות רטוב'. תמיד הילדים מ-ח'2 רואים וצוחקים. 🚧",
            '⚠️': "נכנסת לאזור בשיפוצים! המנהל יטפל בך. ⚠️",
            '🏸': "חטפת כדור נוצה לפנים. וזה משפיל. 🏸",
            '🥏': "צלחת מעופפת לראש! מי בכלל משחק את זה בשיעור ספורט? 🥏",
            '🏐': "חטפת כדורעף לפרצוף מאלה שתמיד מגזימים בכוח. 🏐",
            '🎾': "המורה צעק עליך בטניס. עזוב, אתה יותר בקטע של שחמט. 🎾",
            '⚽': [
                "הכדורגל של השמיניסטים עף ישר אליך. כואב. ⚽",
                "פספסת בעיטה מול שער ריק בהפסקה. כולם ראו. ⚽"
            ],
            '🏀': [
                "ניסית להטביע והפלסטיק של הסל נשבר עליך. 🏀",
                "מסרו לך כדור חזק מדי והאצבע שלך נשברה לאחור. קלאסי. 🏀"
            ],
            '🎳': "כדור כוח על הרגל! הלך האצבע. 🎳",
            '🏋️‍♂️': "ניסית להרים משקולות כדי להרשים את המורות. נתפס הגב. 🏋️‍♂️",
            '👟': [
                "נשמת את הריח מהנעליים באולם ספורט. סכנת חיים. 👟",
                "קשרו לך את השרוכים אחד לשני ונפלת בריצה. 👟"
            ],
            '🦅': "נשר צלל עליך כי חשב שאתה חטיף בטיול השנתי. 🦅",
            '🦇': "נכנסת למערה והערת עטלף. עכשיו אתה בבידוד לכלבת. 🦇",
            '🐝': "דבורה עקצה אותך בטיול. כולם המשיכו ואתה נשארת עם החובש. 🐝",
            '🦟': "יתוש של מצדה מצץ לך את כל הדם בטיול השנתי. 🦟",
            '🐍': "שיחקת אותה גבר עם 'נחש חולות', בסוף הכיש אותך. 🐍",
            '🦂': "עקרב נכנס לך לשק שינה. ביי. 🦂",
            '🌵': "הישענת על קקטוס בשביל תמונה לאינסטגרם. 🌵",
            '🥾': "נקרע לך הסנדל שורש. נשארת באוהל עם המורות. 🥾",
            '🧪': [
                "ערבבת אדום עם ירוק. המעבדה התפוצצה. 🧪",
                "שתית בטעות מהכוס של הניסוי כי היית צמא. 🧪"
            ],
            '🦠': "עשית ניסוי ביולוגיה ושכחת לשטוף ידיים. 🦠",
            '🛸': "הכנתם חייזר במקום פרויקט מדעים, המורה לא זרמה. 🛸",
            '⚡': "הכנסת עיפרון לשקע במעבדה 'רק לבדוק'. ⚡",
            '☄️': "משהו נשרף במנדף. כל בית הספר פונה. ☄️",
            '🔬': "הסתכלת במיקרוסקופ והייתה שם עין של ג'וק. התעלפת. 🔬",
            '💥': "המורה לכימיה הדליקה אש ליד גז פתוח. 💥",
            '🎵': "שמעת עומר אדם על פול ווליום, רכזת השכבה החרימה את הרמקול. 🎵",
            '🎶': "המורה שאלה שאלה, ואתה שרת לה במקום לענות. 🎶",
            '🎮': "נתפסת משחק בראול סטארס מתחת לשולחן בשיעור תנ'ך. 🎮",
            '🌨️': [
                "המזגן על 16. קפאת, עכשיו אתה גוש קרח שמוצג במסדרון. 🌨️",
                "ביקשת לכבות את המזגן והבנים זרקו עליך קלמרים. 🌨️"
            ],
            '🌬️': "הרוח מהמזגן כל כך חזקה שהיא העיפה לך את המבחן. 🌬️",
            '🧊': "הייתה טפטפת מהמזגן, עכשיו אתה גם רטוב וגם קפוא. 🧊",
            '❄️': "הבנים כיוונו את המזגן ישר עליך, קפאת למוות. ❄️",
            '🪁': "עפת ברוח החוצה מרוב שהמזגן חזק. 🪁",
            '☔': "פתחת מטריה בכיתה כי המזגן מטפטף. המורה העיפה אותך. ☔",
            '🌂': "ניסית להתגונן מהטפטוף של המזגן, אבל הוא ניצח. 🌂",
            '👨‍💼': [
                "המנהל קלט אותך במסדרון: 'תגיד לי חמוד, לאן אתה הולך?!' 👨‍💼",
                "המנהל תפס אותך מתגנב לשער בזמן שיעור מתמטיקה. 👨‍💼"
            ],
            '👮‍♂️': [
                "המאבטח בכניסה לא האמין שאתה לומד פה ולא נתן לך להיכנס. 👮‍♂️",
                "המאבטח החרים לך את הלייזר פוינטר שהבאת מחו'ל. 👮‍♂️"
            ],
            '🛑': "תורנות חצר מנעה ממך לעבור בהפסקה. 🛑",
            '🚨': "האזעקה של בית הספר הופעלה כי נשענת על הדלת הלא נכונה. 🚨",
            '👻': "שומר הלילה חשב שאתה רוח רפאים והעיף אותך. 👻",
            '💀': "השלד של מעבדת ביולוגיה נפל עליך בחושך. 💀",
            '👽': "חייזרים נחתו בבית הספר. האמת, אחלה תירוץ. 👽",
            '🕷️': "ג'וק מעופף בחושך!!! צרחת והערת את השכונה. 🕷️",
            '🦉': "ינשוף הסתכל עליך מבחוץ ב-2 בלילה. קריפי מדי. 🦉",
            '🔦': "הפנס של השומר תפס אותך מנסה להבריז בלילה. 🔦",
            '✈️': "פספסת את הטיסה כי היית צריך להחזיר ספר לספרייה. ✈️",
            '📓': "קיבלת חוברת עבודה לחופש!!! זה יותר גרוע מלמות. 📓",
            '👩‍🏫': "המורה תפסה אותך בשער ונתנה לך עבודת קיץ 'כי יש לך פוטנציאל'. 👩‍🏫",
            '🏫': "מרוב שהתרגשת לחופש, רצת לתוך דלת זכוכית. 🏫",
            '🎈': "בלון סיום השנה התפוצץ לך בפרצוף. 🎈"
        };
        
        let msg = null;
        if (killerEmoji && msgMap[killerEmoji]) {
            msg = msgMap[killerEmoji];
        ...
        const msgs = [
            ...
            "האוטובוס ברח לך! 🚌💨", 
            "2 + 2 = 5? חזור על החומר! 📚", 
            "הסנדוויץ' נפל עם הממרח למטה! 🥪💥", 
            "נפסלת במחניים! 🏐", 
            "נפל לך הסנדוויץ' לבוץ! 🥾", 
            "הניסוי התפוצץ במעבדה! 💥🧪", 
            "המורה תפסה אותך משחק בטלפון! 📱", 
            "קפאת למוות, מי שם על 16 מעלות?! 🥶", 
            "המנהל קורא לך למשרד! 👨‍💼", 
            "הדלתות ננעלו! 👻", 
            "נשרפת בים, שכחת קרם הגנה! 🥵☀️" 
        ];
