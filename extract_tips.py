...
def extract_from_txt():
    txt_path = r"C:\Users\user\נטו חופש\neto-hofesh\raw_tips.txt"
    ...
    # Split text into lines, but wait - there are page separators like "==End of OCR for page 1==\n\n==Screenshot for page 2==\n\n==Start of OCR for page 2=="
    lines = full_text.split('\n')
    cleaned_lines = []
    for line in lines:
        if line.startswith("=="): continue
        if line.strip() == "": continue
        # some lines start with r",\n", let's just keep the text
        if line.startswith('",'): continue
    ...
    def process_block(date_key, block_lines):
        ...
        if "משימה יומית" in text or "אופציה לחטיבה" in text:
            ...
            m_high = re.search(r'\[אופציה לחטיבה ותיכון\]:(.*?)(?=\[אופציה ליסודי\]|טיפ החג|טיפ החופש|\Z)', text, re.DOTALL)
            if m_high: mission_high = m_high.group(1).strip()
            
            m_elem = re.search(r'\[אופציה ליסודי\]:(.*?)(?=טיפ החג|טיפ החופש|\Z)', text, re.DOTALL)
            if m_elem: mission_elem = m_elem.group(1).strip()
            
            m_tip = re.search(r'טיפ (?:החג|החופש הגדול|החופש):?(.*?)\Z', text, re.DOTALL)
            if m_tip: tip = m_tip.group(1).strip()
            
            # fallback if standard regex didn't catch things fully
            if not mission_high and not mission_elem and "משימה יומית" in text:
                m_general = re.search(r'משימה יומית.*?:(.*?)(?=טיפ החג|טיפ החופש|\Z)', text, re.DOTALL)
                if m_general:
                    mission_high = m_general.group(1).strip()
    ...
    for line in cleaned_lines:
        ...
        if m and "–" not in line.split(')')[0] and "-" not in line.split(')')[0]:
            ...
            month_raw = m.group(2)
            if month_raw.startswith('ב') and month_raw not in ["במאי"]:
                month_raw = month_raw[1:]
            month = months.get(month_raw, "00")
